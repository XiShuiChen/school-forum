...
public class WeatherServiceImpl implements WeatherService {
    ...
    private WeatherVO fetchFromCache(double longitude, double latitude) {
        JSONObject geo = this.decompressStringToJSON(rest.getForObject("https://geoapi.qweather.com/v2/city/lookup?location="
    ...
    private WeatherVO fetchFromAPI(int id, JSONObject location) {
        ...
        JSONObject now = this.decompressStringToJSON(rest.getForObject("https://devapi.qweather.com/v7/weather/now?location="
                + id + "&key=" + key, byte[].class));
        if (now == null) return null;
        vo.setNow(now.getJSONObject("now"));

        JSONObject hourly = this.decompressStringToJSON(rest.getForObject("https://devapi.qweather.com/v7/weather/24h?location="
                + id + "&key=" + key, byte[].class));
    ...
    private JSONObject decompressStringToJSON(byte[] data) {
        ...
        try {
            ...
            int read;
            while ((read = gzip.read(buffer)) != -1) stream.write(buffer, 0, read);
            gzip.close();
            stream.close();

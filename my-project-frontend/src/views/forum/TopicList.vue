    ...
    <div style="flex: 1">
      ...
      <light-card style="margin-top: 10px; display: flex; flex-direction: column; gap: 10px">
        <div v-for="item in topics.top" class="top-topic" @click="router.push(`/index/topic-detail/${item.id}`)">
          <el-tag style="transform: translateY(1px)" type="danger" size="small">置顶</el-tag>
          <div>{{ item.title }}</div>
        ...
        <div v-if="topics.list.length">
          <div style="margin-top: 10px; display: flex; flex-direction: column;gap: 10px"
            ...
            <light-card v-for="item in topics.list" class="topic-card"
              ...
              <div style="display: flex; gap: 20px; font-size: 13px; margin-top: 10px; opacity: 0.9">
                <div>
                  <el-icon style="vertical-align: -10%"><CircleCheck/></el-icon> {{ item.like }}点赞
                </div>
                <div>
                  <el-icon style="vertical-align: -10%"><Star/></el-icon> {{ item.collect }}收藏
                </div>
              </div>
            </light-card>
          </div>
